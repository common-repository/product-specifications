...
final class ImportDataAjaxHandler
{
    ...
    public function __invoke(): void
    {
        ...
        $nonce = sanitize_text_field(wp_unslash((string) filter_input(\INPUT_POST, 'dws_im_nonce', \FILTER_SANITIZE_SPECIAL_CHARS)));
        if (!(bool) wp_verify_nonce($nonce, 'dwspecs_nonce_import')) {
            wp_send_json_error(['message' => esc_html__('Invalid request', 'product-specifications')]);
        }
        // Exit if already migrating
        if (\false !== get_transient('dwspecs_data_migrating')) {
            wp_send_json_error(['message' => esc_html__('Another Migration is in progress', 'product-specifications')]);
        }
        $fileInput = $_FILES['file'] ?? null;
        // phpcs:ignore
        if (is_null($fileInput)) {
            wp_send_json_error(['message' => esc_html__('Invalid file', 'product-specifications')]);
        }
        if ($fileInput['type'] !== 'application/json' || !$fileInput['size']) {
            wp_send_json_error(['message' => esc_html__('Invalid File', 'product-specifications')]);
        }
        $data = json_decode((string) file_get_contents($fileInput['tmp_name']), \true);
        if (json_last_error() !== \JSON_ERROR_NONE) {
            wp_send_json_error(['message' => esc_html__('Invalid File', 'product-specifications')]);
        }
        set_transient('dwspecs_data_migrating', \true, MINUTE_IN_SECONDS * 30);
        // Migrate Plugin data
        $this->importPluginData((array) $data);
        delete_transient('dwspecs_data_migrating');
        wp_send_json_success(['message' => esc_html__('Data Imported successfully', 'product-specifications')]);
        wp_die();
    }
    ...
    private function importPluginData(array $data): void
    {
        ...
        foreach ($data as $table) {
            ...
            $existing_table = $wpdb->get_var($wpdb->prepare("SELECT ID FROM {$wpdb->posts} WHERE post_name = %s AND post_type= %s AND post_status = 'publish'", $table['table-slug'], 'specs-table'));
            if ($existing_table) {
                $new_table_id = $existing_table;
            } else {
                $new_table_id = wp_insert_post(['post_type' => 'specs-table', 'post_title' => $table['table-title'], 'post_name' => $table['table-slug'], 'post_status' => 'publish', 'meta_input' => ['old_id' => $table['table-id']]]);
            }
            foreach ($table['skleton'] as $group) {
                ...
                if ($existing_group = get_term_by('slug', $group['slug'], 'spec-group')) {
                    ...
                    // Create new
                    $new_group_id = wp_insert_term($group['name'], 'spec-group', ['slug' => $group['slug']]);
                ...
                foreach ($group['attributes'] as $attribute) {
                    ...
                    if ($existing_attribute = get_term_by('slug', $attribute['slug'], 'spec-attr')) {
                        ...
                        // Create new
                        $new_attr_id = wp_insert_term($attribute['title'], 'spec-attr', ['slug' => $attribute['slug']]);
            ...
            if (isset($table['products'])) {
                foreach ($table['products'] as $product) {
                    ...
                    // #End modify IDs
                    // Get product by slug
                    $product_id = $wpdb->get_var($wpdb->prepare("SELECT ID FROM {$wpdb->posts} WHERE post_name = %s AND post_type= %s AND post_status = 'publish'", $product['slug'], 'product'));
                    if ($product_id) {
