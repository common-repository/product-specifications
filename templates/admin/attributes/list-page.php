...
/* namespace { PHP-SCOPER: Namespace removed intentionally */
     ...
     * @var array $data
     */
    ['collection' => $collection, 'searchKeyword' => $searchKeyword, 'groupId' => $groupId] = $data;
    ?>

<div class="dwps-page">
    <div class="dwps-settings-wrap">
        <h3><?php 
    echo \esc_html__('Attributes', 'product-specifications');
    ?></h3>
    ...
    ?></th>
                        <th><?php 
    echo \esc_html__('Attribute name', 'product-specifications');
    ?></th>
                        <th><?php 
        ...
        foreach ($collection as $attribute) {
                            ...
                            <tr>
                                <td class="check-column">
                                    <input
                                        ...
                                        name="slct_group[]"
                                        value="<?php 
            echo \esc_attr($attribute->term_id);
            ?>"
                                    />
                                </td>

                                <td><?php 
            echo \esc_html($attribute->term_id);
            ?></td>
            ...
            ?>
                                ...
                                <td>
                                    <h4>
                                        <?php 
            $group = \get_term(\get_term_meta($attribute->term_id, 'attr_group', \true), 'spec-group');
            if (!\is_wp_error($group)) {
                ?>
                                            <a
                                                href="<?php 
                echo \esc_url(\add_query_arg('group_id', $group->term_id, \remove_query_arg('paged')));
                ?>"
                                            >
                                                <?php 
    ...
    ?>
            ...
            <div class="dwps-buttons clearfix">
                <a id="dpws_bulk_delete_btn" class="dwps-button red-btn delete-selecteds-btn" href="#" role="button" aria-label="<?php 
    ...
    echo \esc_attr__('Add a new attribute', 'product-specifications');
    ?>"><?php 
...
<script id="dwps_delete_template" type="x-tmpl-mustache" data-templateType="JSON">
    {
        ...
        "modal" : {
            "title" : "<?php 
    echo \esc_attr__("Delete Attribute", "product-specifications");
    ?>",
            "content" : "<?php 
    echo \esc_attr__("Are you sure you want to delete selected attribute(s)?", "product-specifications");
    ?>",
            "buttons" : {
                "primary": {
                    "value": "<?php 
    echo \esc_attr__("Yes", "product-specifications");
    ?>",
                    "href": "#",
                ...
                "secondary": {
                    "value": "<?php 
    echo \esc_attr__("No", "product-specifications");
    ?>",
...
<script id="dwps_texts_template" type="x-tmpl-mustache">
    ...
    ?>",
        "add_title"   : "<?php 
    echo \esc_attr__('Add new attribute', 'product-specifications');
    ?>",
        "edit_title"  : "<?php 
    echo \esc_attr__('Edit attribute', 'product-specifications');
    ?>"
    }
</script>
...
<script id="modify_form_template" type="x-tmpl-mustache">
    ...
    ?></label>
        ...
        <p>
            <label for="attr_slug"><?php 
    echo \esc_html__('Attribute slug : ', 'product-specifications');
    ?></label>
            <input type="text" name="attr_slug" value="" id="attr_slug" placeholder="<?php 
    echo \esc_attr__('Optional', 'product-specifications');
    ?>">
        </p>

    <p>
        <label for="attr_group"><?php 
    echo \esc_html__('Attribute group : ', 'product-specifications');
    ?></label>
            <select name="attr_group" id="attr_group" aria-required="true">
                <option value=""><?php 
    echo \esc_html__('Select a group', 'product-specifications');
    ?></option>
    <?php 
    $all_groups = \get_terms(['taxonomy' => 'spec-group', 'hide_empty' => \false]);
...
<p>
    <label for="attr_type"><?php 
    echo \esc_html__('Attribute field Type : ', 'product-specifications');
    ?></label>
            <select name="attr_type" id="attr_type" aria-required="true">
                <option value=""><?php 
    echo \esc_html__('Select a field type', 'product-specifications');
    ?></option>
                <option value="text"><?php 
    echo \esc_html__('Text', 'product-specifications');
    ?>
    <option value="select"><?php 
    echo \esc_html__('Select', 'product-specifications');
    ?>
    <option value="radio"><?php 
    echo \esc_html__('Radio', 'product-specifications');
    ?>
    <option value="textarea"><?php 
    echo \esc_html__('Textarea', 'product-specifications');
    ?>
    <option value="true_false"><?php 
    echo \esc_html__('True/false', 'product-specifications');
    ?>
    </select>
</p>

<p style="display:none;">
    <label for="attr_values"><?php 
    echo \esc_html__('Values : ', 'product-specifications');
    ?></label>
            <textarea name="attr_values" id="attr_values"></textarea>
        </p>

        <p>
            <label for="attr_default"><?php 
    echo \esc_html__('Default value : ', 'product-specifications');
    ?></label>
            <span id="default_value_wrap"><input type="text" name="attr_default" id="attr_default" value=""></span>
        </p>

        <p>
            <label for="attr_desc"><?php 
    echo \esc_html__('Description : ', 'product-specifications');
    ?></label>
            <textarea name="attr_desc" id="attr_desc" placeholder="<?php 
    echo \esc_attr__('Optional', 'product-specifications');
    ?>"></textarea>
        </p>
    ...
    <?php 
    \wp_nonce_field('dwps_modify_attributes', 'dwps_modify_attributes_nonce');
    ?>
    <input type="submit" value="<?php 
    echo \esc_attr__('Add attribute', 'product-specifications');
